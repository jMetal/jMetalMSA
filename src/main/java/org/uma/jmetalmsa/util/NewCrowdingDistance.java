 ...
 */
public class NewCrowdingDistance<S extends Solution<?>>
  ...
  public void computeDensityEstimator(List<S> solutionList) {
    ...
    double [][] distance = SolutionListUtils.distanceMatrix_normalize(front,objetiveMaxn,objetiveMinn);
    ...
    for (int i = 0; i < distance.length; i++) {
      ...
      {
          kDistance += (dmaxx-dminn) / (distance[i][k]+(dminn));//dmaxx-dmin
      }
